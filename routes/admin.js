...
router.get('/users', async(req,res)=>{
    try {
        const getUsers = await pool.query(`SELECT * FROM public."USER_INFO"`)
        res.status(200).json({
            body:getUsers.rows,
...
router.get('/users/page/:id', async(req,res)=>{
    try {
        const getUsers = await pool.query(`SELECT * FROM public."USER_INFO" WHERE "User_ID" =  $1 `,[req.params.id]);
        const getProject = await pool.query(`SELECT * FROM public."FUNDRAISER_INFO" WHERE "User_ID" = $1`, [req.params.id]);
        res.status(200).json({
            body:{
                user:getUsers.rows,
